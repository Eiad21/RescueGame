...
public class Simulator implements WorldListener {
	...
	public String toString() {
		String s = "Game Simulation Info:" + "\n\t- Current Cycle: "
				+ currentCycle + "\n\t- #Casualties: " + calculateCasualties()
				+ "\n\t- Executed Disasters: " + executedDisasters.size();
		for (int i = 0; i < executedDisasters.size(); i++) {
			s = s + "\n\t" + (i + 1) + ") "
					+ executedDisasters.get(i).getDisasterType()
					+ " - Active State : "
	...
	public String getLog() {
		String s = "Cycle (" + currentCycle + "):"+
					"\n- #Casualties so far (" + calculateCasualties() + "):";
		for (int i = 0; i < citizens.size(); i++) {
			if (citizens.get(i).getState() == CitizenState.DECEASED) {
		...
		s = s +	"\n- Executed Disasters so far (" + executedDisasters.size() + "):";
		for (int i = 0; i < executedDisasters.size(); i++) {
			s = s + "\n\t" + (i + 1) + ") "
					+ executedDisasters.get(i).getDisasterType()
					+ " - Active State : "
